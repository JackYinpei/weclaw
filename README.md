...
- **多会话 Switch Bar**。聊天页内置标签切换栏，支持创建多个独立会话（每个会话拥有独立上下文），以及快速切换到群聊房间，无需返回 Dashboard。
- **群聊房间 & @mention**。多用户多 Agent 群聊，通过 @AgentName 召唤指定 Agent 回复，每个 Agent 维护独立会话上下文。支持邀请平台用户加入房间。
...
- **共享知识库**。通过宿主机目录 Bind Mount 到所有容器，管理员放入文件即可全局共享。
- **Web 搜索集成**。可选配置 Kimi 等搜索引擎，让 AI 助手具备联网搜索能力，获取实时信息。
...
2. **配置：** 编辑 `configs/config.yaml` 或设置环境变量（`WECLAW_OPENCLAW_API_KEY` 等）。
   - **Web 搜索（可选）：** 在 `openclaw.web_search` 中启用搜索并填入 API Key，配置后 AI 助手即可联网搜索。
3. **运行：**
   ```console
   # 开发调试
   ...
   ./bin/weclaw
   ```
   > **注意：** 如果当前用户不是 root，需要使用 `sudo` 运行。因为 OpenClaw 容器内以 `node` 用户（UID 1000）运行，WeClaw 需要 root 权限将 Bind Mount 目录 chown 为 UID 1000，否则容器内会出现 `EACCES: permission denied` 错误。
   > ```console
...
对话界面顶部的 **Switch Bar** 支持创建多个独立会话标签。点击 **+ New** 创建新会话，不同会话之间上下文完全隔离。

### 5. 群聊

在 Switch Bar 点击 **+ Room** 创建群聊房间，或点击 **Join** 加入已有房间。群聊中通过 `@AgentName` 召唤指定 Agent 回复。点击成员栏的 **+ Invite** 按钮可邀请其他平台用户。
...
| 流式 | GET | `/ws/containers/:id?token=JWT` | WebSocket 流式对话（支持 session_tag 多会话） |
| 流式 | GET | `/ws/rooms/:roomId?token=JWT` | 群聊 WebSocket（@mention 多 Agent） |
...
| 群聊 | GET | `/api/rooms` | 列出已加入的房间 |
| 群聊 | POST | `/api/rooms` | 创建房间 |
| 群聊 | DELETE | `/api/rooms/:roomId` | 删除房间 |
| 群聊 | POST | `/api/rooms/:roomId/join` | 加入房间 |
| 群聊 | POST | `/api/rooms/:roomId/invite` | 邀请用户 |
| 群聊 | POST | `/api/rooms/:roomId/leave` | 离开房间 |
| 群聊 | GET | `/api/rooms/:roomId/members` | 房间成员 |
| 群聊 | GET | `/api/rooms/:roomId/messages` | 房间消息历史 |

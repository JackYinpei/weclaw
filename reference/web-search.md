...
llm-context returns extracted page chunks for grounding instead of standard Brave snippets. In this mode, country and language / search_lang still work, but ui_lang, freshness, date_after, and date_before are rejected.
Perplexity Search:
{
  tools: {
    web: {
...
Perplexity’s OpenRouter / Sonar compatibility path supports only query and freshness. If you set tools.web.search.perplexity.baseUrl / model, use OPENROUTER_API_KEY, or configure an sk-or-... key, Search API-only filters return explicit errors.
